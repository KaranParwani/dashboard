...
from patient.schemas.patient import PatientRecord
...
async def get_patient_details(
    ...
    try:
        ...
        print(authenticate.authenticate_admin_token(authorization))
...
async def delete_patient_record(patient_id: int = None, admin_id: int = None, session: Session = Depends(db_manager.get_database_session),
    authorization: str = Header(None)):
    try:
        authenticate = AdminAuthenticator(session, SECRET_KEY, ALGORITHM)
        authenticate.authenticate_admin_token(authorization)
        ...
        patients = PatientsManager(session)
        return await patients.delete_patient_record(patient_id, admin_id)
